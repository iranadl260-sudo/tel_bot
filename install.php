<?php
/**
 * نصب‌کننده ربات تلگرام - نسخه 14 نهایی
 * با عیب‌یاب کاملاً جامع
 */
...
class Installer {
    ...
    private function runFullDiagnostic() {
        ...
        $this->diagnosticResults[] = [
            ...
            'fix' => 'فعال‌سازی pdo_mysql'
        ];
        $this->diagnosticResults[] = [
            'section' => 'سرور',
            'name' => 'cURL',
            'status' => extension_loaded('curl'),
            'detail' => extension_loaded('curl') ? 'فعال' : 'غیرفعال',
            'fix' => 'فعال‌سازی curl'
        ];
        $this->diagnosticResults[] = [
            'section' => 'سرور',
            ...
            'detail' => is_writable(__DIR__) ? 'مجاز' : 'غیرمجاز',
            'fix' => 'chmod 755'
        ];
        
        // ۲. فایل‌ها
        $files = ['config.php', 'database.php', 'bot.php', 'index.php', 'admin.php'];
        foreach ($files as $f) {
        ...
        if (file_exists(__DIR__ . '/config.php')) {
            ...
            $this->diagnosticResults[] = [
                'section' => 'تنظیمات',
                'name' => 'توکن',
                'status' => defined('BOT_TOKEN') && !empty(BOT_TOKEN),
                'detail' => defined('BOT_TOKEN') && !empty(BOT_TOKEN) ? substr(BOT_TOKEN, 0, 15) . '...' : 'خالی',
                'fix' => 'در admin.php توکن را وارد کنید'
            ];
            
            $this->diagnosticResults[] = [
                'section' => 'تنظیمات',
                'name' => 'ADMIN_IDS',
                'status' => defined('ADMIN_IDS') && is_array(ADMIN_IDS) && !empty(ADMIN_IDS),
                'detail' => defined('ADMIN_IDS') && !empty(ADMIN_IDS) ? implode(',', ADMIN_IDS) : 'خالی',
                'fix' => 'آیدی عددی را وارد کنید'
            ];
            
            $this->diagnosticResults[] = [
                'section' => 'تنظیمات',
                'name' => 'دیتابیس',
                'status' => defined('DB_NAME') && !empty(DB_NAME),
                'detail' => defined('DB_NAME') ? DB_NAME : 'خالی',
                'fix' => 'اطلاعات دیتابیس را وارد کنید'
            ];
            ...
            if (defined('BOT_TOKEN') && !empty(BOT_TOKEN)) {
                ...
                if ($curlError) {
                    $this->diagnosticResults[] = [
                        'section' => 'API تلگرام',
                        'name' => 'اتصال به api.telegram.org',
                        'status' => false,
                        'detail' => $curlError,
                        'fix' => 'سرور دسترسی ندارد - هاست را عوض کنید'
                    ];
                } else {
                    $this->diagnosticResults[] = [
                        'section' => 'API تلگرام',
                    ...
                    if ($data && $data['ok']) {
                        $this->diagnosticResults[] = [
                            ...
                            'name' => 'اعتبار توکن',
                            'status' => true,
                            'detail' => $data['result']['first_name'] . ' (@' . $data['result']['username'] . ')',
                            'fix' => ''
                        ];
                    } else {
                        $this->diagnosticResults[] = [
                            ...
                            'name' => 'اعتبار توکن',
                            'status' => false,
                            'detail' => $data['description'] ?? 'نامعتبر',
                            'fix' => 'از @BotFather توکن جدید بگیرید'
                        ];
                    }
                }
                
                // ۵. تست Webhook
                $whUrl = 'https://api.telegram.org/bot' . BOT_TOKEN . '/getWebhookInfo';
                $whResp = @file_get_contents($whUrl);
                ...
                if ($whResp) {
                    ...
                    if ($whData['ok']) {
                        ...
                        $this->diagnosticResults[] = [
                            'section' => 'Webhook',
                            'name' => 'آدرس Webhook',
                            'status' => !empty($whInfo['url']),
                            'detail' => $whInfo['url'] ?: 'خالی',
                            'fix' => 'Webhook را تنظیم کنید'
                        ];
                        
                        if (!empty($whInfo['last_error_message'])) {
                            $this->diagnosticResults[] = [
                                'section' => 'Webhook',
                                'name' => 'آخرین خطا',
                                'status' => false,
                                'detail' => $whInfo['last_error_message'],
                                'fix' => $this->getWebhookFix($whInfo['last_error_message'])
                            ];
                        }
                        ...
                        $this->diagnosticResults[] = [
                            ...
                            'name' => 'پیام‌های در صف',
                            'status' => true,
                            'detail' => $whInfo['pending_update_count'] . ' پیام',
                            'fix' => ''
                        ];
                    }
                }
                ...
                if (defined('ADMIN_IDS') && !empty(ADMIN_IDS)) {
                    $adminId = ADMIN_IDS[0];
                    $testUrl = 'https://api.telegram.org/bot' . BOT_TOKEN . '/sendMessage?chat_id=' . $adminId . '&text=' . urlencode('✅ تست - ' . date('H:i:s'));
                    $testResp = @file_get_contents($testUrl);
                    ...
                    if ($testResp) {
                        ...
                        if ($testData['ok']) {
                            ...
                            $this->diagnosticResults[] = [
                                ...
                                'name' => 'تست ارسال به ادمین',
                                'status' => false,
                                'detail' => $testData['description'],
                                'fix' => 'ابتدا به ربات /start بزنید'
                            ];
                        }
                    }
                }
                ...
                try {
                    ...
                    if ($db->isConnected()) {
                        $this->diagnosticResults[] = [
                            ...
                            'name' => 'اتصال',
                            'status' => true,
                            'detail' => DB_NAME . ' (پیشوند: ' . DB_PREFIX . ')',
                            'fix' => ''
                        ];
                        ...
                        $this->diagnosticResults[] = [
                            ...
                            'name' => 'آمار',
                            'status' => true,
                            'detail' => "{$stats['files']} فایل | {$stats['users']} کاربر | {$stats['downloads']} دانلود",
                            'fix' => ''
                        ];
                    } else {
                        $this->diagnosticResults[] = [
                            ...
                            'status' => false,
                            'detail' => 'متصل نیست',
                            'fix' => 'اطلاعات دیتابیس را بررسی کنید'
                        ];
                    }
                } catch (Exception $e) {
                    $this->diagnosticResults[] = [
                        ...
                        'status' => false,
                        'detail' => $e->getMessage(),
                        'fix' => 'اطلاعات دیتابیس را اصلاح کنید'
                    ];
                }
            }
        }
        ...
        // ۸. لاگ‌ها
        $logs = ['logs/errors.log', 'logs/requests.log'];
        foreach ($logs as $log) {
            $logPath = __DIR__ . '/' . $log;
            if (file_exists($logPath)) {
                ...
                $this->diagnosticResults[] = [
                    ...
                    'status' => false,
                    'detail' => 'وجود ندارد',
                    'fix' => 'به ربات پیام بدهید تا لاگ ایجاد شود'
                ];
            }
        }
    }
    
    private function getWebhookFix($error) {
        if (strpos($error, 'timeout') !== false) return 'سرور کند است - از run.php استفاده کنید';
        if (strpos($error, '404') !== false) return 'فایل index.php در مسیر Webhook نیست';
        if (strpos($error, '500') !== false) return 'خطای سرور - لاگ خطاها را بررسی کنید';
        if (strpos($error, 'SSL') !== false) return 'مشکل SSL - گواهی را بررسی کنید';
        return 'Webhook را دوباره تنظیم کنید';
    }
...
echo '<!DOCTYPE html><html dir=\"rtl\"><head><meta charset=\"UTF-8\"><title>عیب‌یاب کامل</title>';
...
echo '<h1>🔍 عیب‌یاب کامل ربات</h1>';
echo '<p>' . date('Y-m-d H:i:s') . '</p>';

// ۱. سرور
echo '<h2>۱. بررسی سرور</h2>';
...
if (file_exists(__DIR__ . '/config.php')) {
    ...
    echo '<p>' . (defined('ADMIN_IDS') && is_array(ADMIN_IDS) && !empty(ADMIN_IDS) ? '✅' : '❌') . ' ADMIN_IDS</p>';
    echo '<p>' . (defined('DB_NAME') && !empty(DB_NAME) ? '✅' : '❌') . ' دیتابیس</p>';
    
    // ۴. API
    echo '<h2>۴. تست API تلگرام</h2>';
    if (defined('BOT_TOKEN') && !empty(BOT_TOKEN)) {
        ...
        if (\$err) {
            echo '<p class=\"fail\">❌ ' . \$err . '</p>';
            echo '<div class=\"fix\">🔧 سرور به api.telegram.org دسترسی ندارد</div>';
        } else {
            \$data = json_decode(\$resp, true);
            if (\$data['ok']) {
                echo '<div class=\"ok\">✅ ' . \$data['result']['first_name'] . ' (@' . \$data['result']['username'] . ')</div>';
            } else {
                echo '<p class=\"fail\">❌ ' . \$data['description'] . '</p>';
            }
        }
        ...
        if (\$whResp) {
            ...
            if (\$whData['ok']) {
                ...
                echo '<p>📊 صف: ' . \$info['pending_update_count'] . '</p>';
            }
        }
        
        // ۶. تست پیام
        echo '<h2>۶. تست ارسال پیام</h2>';
        if (defined('ADMIN_IDS') && !empty(ADMIN_IDS)) {
            \$adminId = ADMIN_IDS[0];
            \$tUrl = 'https://api.telegram.org/bot' . BOT_TOKEN . '/sendMessage?chat_id=' . \$adminId . '&text=' . urlencode('✅ تست - ' . date('H:i:s'));
            \$tResp = @file_get_contents(\$tUrl);
            if (\$tResp) {
    ...
    if (file_exists(__DIR__ . '/database.php') && defined('DB_NAME') && !empty(DB_NAME)) {
        ...
        try {
            \$db = Database::getInstance();
            if (\$db->isConnected()) {
                echo '<div class=\"ok\">✅ متصل به ' . DB_NAME . '</div>';
                \$stats = \$db->getStats();
                echo '<p>📊 ' . \$stats['files'] . ' فایل | ' . \$stats['users'] . ' کاربر | ' . \$stats['downloads'] . ' دانلود</p>';
            } else {
                echo '<p class=\"fail\">❌ متصل نیست</p>';
            }
        } catch (Exception \$e) {
    ...
    foreach (\$logs as \$log) {
        \$lp = __DIR__ . '/' . \$log;
        if (file_exists(\$lp)) {
            \$lc = file_get_contents(\$lp);
            echo '<p>✅ ' . \$log . ' (' . substr_count(\$lc, \"\\n\") . ' خط)</p>';
            if (!empty(\$lc)) {
                echo '<pre style=\"background:#f5f5f5;padding:10px;max-height:150px;overflow:auto;font-size:11px\">' . htmlspecialchars(substr(\$lc, -1000)) . '</pre>';
            }
        } else {
            echo '<p class=\"fail\">❌ ' . \$log . ' وجود ندارد</p>';
        }
    }
}
    ...
    public function render() {
        ...
        <div class="container">
            ...
            <?php if ($this->step == 1): ?>
                ...
                <div style="text-align:center;margin-top:10px"><a href="?diagnostic=1" class="link-btn">🔍 فقط عیب‌یابی</a></div>
                ...
                <table class="diag-table">
                    ...
                    <?php foreach ($this->diagnosticResults as $r): ?>
                        <tr>
                            ...
                            <td class="<?php echo $r['status'] ? 'pass' : 'fail'; ?>"><?php echo $r['status'] ? '✅' : '❌'; ?></td>
                            <td><?php echo $r['detail']; ?></td>
                            <td><?php echo $r['fix']; ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
