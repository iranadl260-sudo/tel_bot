...
/**
 * ============================================================
 * 🤖 نصب‌کننده خودکار ربات تلگرام
 * نسخه: 12.0 - مخصوص Render و هاست‌های ابری
 * ============================================================
 */
...
class RenderInstaller {
    ...
    private function checkServer() {
        $checks = [
            ...
            'دسترسی نوشتن' => is_writable(__DIR__)
        ];
    ...
    private function saveConfig() {
        ...
        // تست توکن
        $tokenValid = $this->testToken($_POST['bot_token']);
        if (!$tokenValid) {
            $this->errors[] = "❌ توکن نامعتبر است";
            return;
        }
        
        $_SESSION['config'] = $_POST;
        $this->config = $_POST;
        $this->messages[] = "✅ تنظیمات معتبر است";
        $this->step = 3;
    }
    ...
    private function createAllFiles() {
        $config = $this->config;
    ...
    private function createTables() {
        ...
        try {
            ...
            $this->errors[] = "❌ خطای دیتابیس: " . $e->getMessage();
        }
    }
...
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Tahoma;background:#f0f2f5;padding:20px}.container{max-width:600px;margin:0 auto}.header{background:white;padding:20px;border-radius:15px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:center}.header h1{color:#667eea;font-size:20px}.header a{color:#dc3545;text-decoration:none}.stats{display:grid;grid-template-columns:1fr 1fr 1fr;gap:15px;margin-bottom:20px}.card{background:white;padding:25px;border-radius:10px;text-align:center}.card h3{color:#666;font-size:13px}.card p{font-size:28px;font-weight:bold;color:#667eea}</style>
        ...
        <div class="container">
            ...
            <?php elseif ($this->step == 2): ?>
                <div class="guide">
                    ...
                    <ol>
                        ...
                        <li>دیتابیس: از freesqldatabase.com یا db4free.net</li>
                    </ol>
                </div>
                <form method="POST">
            ...
            <?php elseif ($this->step >= 99): ?>
                ...
                <div class="alert alert-success">
                    ۱. فایل install.php را حذف کنید<br>
                    ۲. به ربات /start بزنید
                </div>
