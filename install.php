<?php
/**
 * نصب‌کننده ربات تلگرام - نسخه 15 نهایی
...
class Installer {
    ...
    public function __construct() {
        ...
        } elseif (isset($_GET['setup_webhook'])) {
            $this->step = 30;
    ...
    private function setupWebhook() {
        $token = $this->config['bot_token'] ?? (defined('BOT_TOKEN') ? BOT_TOKEN : '');
        ...
        if ($response) {
            ...
            if ($data['ok']) {
                $this->messages[] = "✅ Webhook تنظیم شد: {$webhookUrl}";
            } else {
                $this->errors[] = "❌ Webhook: " . $data['description'];
            }
        } else {
            $this->errors[] = "❌ نمی‌توان به API تلگرام متصل شد";
        }
        ...
        if (file_exists(__DIR__ . '/config.php')) {
            ...
            if (defined('BOT_TOKEN') && !empty(BOT_TOKEN)) {
                ...
                if ($whResp) {
                    ...
                    if ($whData['ok']) {
                        $info = $whData['result'];
                        $this->diagnosticResults[] = ['بخش' => 'Webhook', 'تست' => 'URL', 'وضعیت' => !empty($info['url']), 'جزئیات' => $info['url'] ?: 'خالی', 'راه‌حل' => 'اجرای setup_webhook'];
                        if (!empty($info['last_error_message'])) {
                            $this->diagnosticResults[] = ['بخش' => 'Webhook', 'تست' => 'خطا', 'وضعیت' => false, 'جزئیات' => $info['last_error_message'], 'راه‌حل' => 'بررسی فایل index.php'];
                        }
                    }
                }
                ...
                if (defined('ADMIN_IDS') && !empty(ADMIN_IDS)) {
                    ...
                    if ($tResp) {
                        ...
                        $this->diagnosticResults[] = ['بخش' => 'پیام', 'تست' => 'ارسال به ادمین', 'وضعیت' => $tData['ok'], 'جزئیات' => $tData['ok'] ? 'ارسال شد' : $tData['description'], 'راه‌حل' => 'به ربات /start بزنید'];
                    }
                }
            }
            ...
            if (file_exists(__DIR__ . '/database.php') && defined('DB_NAME') && !empty(DB_NAME)) {
                ...
                try {
                    ...
                    $this->diagnosticResults[] = ['بخش' => 'دیتابیس', 'تست' => 'اتصال', 'وضعیت' => $db->isConnected(), 'جزئیات' => $db->isConnected() ? DB_NAME : 'متصل نیست', 'راه‌حل' => 'بررسی اطلاعات دیتابیس'];
                } catch (Exception $e) {
...
class TelegramBot {
    ...
    private function handleMessage(\$message) {
        ...
        if (\$this->isFileMessage(\$message) && isAdmin(\$userId)) { \$this->handleFileUpload(\$chatId, \$message); return; }
        if (strpos(\$text, '/start') === 0) {
            ...
            \$this->sendMessage(\$chatId, \"📊 {\$stats['files']} فایل | {\$stats['users']} کاربر\");
    ...
    private function handleFileUpload(\$chatId, \$message) {
        ...
        if (isset(\$message['document'])) {
            ...
            \$fileData = ['file_id' => \$doc['file_id'], 'file_name' => \$doc['file_name'] ?? 'file_' . time(), 'file_size' => \$doc['file_size'] ?? 0, 'file_type' => strtolower(pathinfo(\$doc['file_name'] ?? '', PATHINFO_EXTENSION)), 'category' => \$category, 'description' => \$caption, 'download_link' => ''];
        } elseif (isset(\$message['video'])) {
            \$vid = \$message['video'];
            \$fileData = ['file_id' => \$vid['file_id'], 'file_name' => 'video_' . time() . '.mp4', 'file_size' => \$vid['file_size'] ?? 0, 'file_type' => 'mp4', 'category' => \$category, 'description' => \$caption, 'download_link' => ''];
        }
        if (\$fileData) {
            try {
        ...
        <div class="container">
            ...
            <?php if ($this->step == 1): ?>
                ...
                <div style="text-align:center;margin-top:10px">
                    ...
                    <a href="?setup_webhook=1" class="link-btn">🌐 تنظیم Webhook</a>
                </div>
